...
workspace_limits = np.asarray([[-0.2, 0.2], [0.7, 0.9], [0.15, 0.3]])

calib_grid_step = 0.05  # 0.05
checkerboard_offset_from_tool = [0, 0.075, 0] # Based on the robot base coordinate system
tool_orientation = [-np.pi / 2, np.pi / 2, 0]  # [0,-2.22,2.22] # [2.22,2.22,0] rpy
# ---------------------------------------------
...
robot = UR_rtde(tcp_host_ip,  workspace_limits, is_use_robotiq=False)
# robot.open_gripper()
...
for calib_pt_idx in range(num_calib_grid_pts):
    ...
    if checkerboard_found:
        ...
        save_path = os.path.join(save_dir, '%06d.png' % len(measured_pts))
